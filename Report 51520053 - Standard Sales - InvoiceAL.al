report 51520053 "Standard Sales - InvoiceAL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Standard Sales - InvoiceAL.rdlc';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(SelltoContact; "Sales Invoice Header"."Sell-to Contact")
            {
            }
            ...
            column(ShiptoName2; "Sales Invoice Header"."Bill-to Address 2")
            {
            }
            column(ExtDocNo; "External Document No.")
            {
            }
            column(Execby; "Sales Invoice Header"."Bill-to County")
            {
            }
            // column(Lpo; "Sales Invoice Header"."Buyer ID/Card No.")
            // {
            // }
            ...
            column(CustomerPIN; Cust2."Telex Answer Back")
            {
            }
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(companyinfopin; CompanyInfo."Contact Person")
                    {
                    }
                    ...
                    column(salespurchcode; SalesPurchPerson.Code)
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(LineAmt_SalesInvLine; Amount)
                        {
                        ...
                        // column(LabelText; "Sales Invoice Line"."Label Text")
                        // {
                        // }
            ...
            trigger OnAfterGetRecord()
            var
                CU: Codeunit 51520051;
                QR: Text[250];
                Setup: Record 51521060;
                CurrencyCode: Code[10];
            begin

                // CurrencyCode := FetChCurrencyCode("No.");


                if "Sales Invoice Header".CUInvoiceNo <> '' then
                    if CU.GetQRASTEXT("Sales Invoice Header") = '' then begin
                        Setup.FindFirst();
                        QR := "Sales Invoice Header".CUInvoiceNo + '.png';

                        //  if (GenerateQRCode(GetJSONValue(J, Setup.ImageServiceUri), )) = 'Success' then begin
                        // Message("Sales Invoice Header".CUInvoiceNo);
                        if (CU.GenerateQRCode(Setup.ImageServiceUri, "Sales Invoice Header".CUInvoiceNo)) = 'Success' then begin
                            "Sales Invoice Header".CalcFields(QRCode);
                            "Sales Invoice Header".QRCode.Import(Setup.QRCodeStorage + QR);
                ...
                Cust2.Get("Sales Invoice Header"."Sell-to Customer No.");
                GETPayBill;
    ...
    local procedure GETPayBill() Paybill: Code[20]
    var
        SPP: Record "Salesperson/Purchaser";
    // Route: Record "District Group Code";
    begin
        //MESSAGE('HEre');
        PB := '';
        if "Sales Invoice Header"."Salesperson Code" <> '' then begin
            // MESSAGE('Press Ok to continue');
            if not SPP.Get("Sales Invoice Header"."Salesperson Code") then
                PB := ''
            else begin
                // Route.Reset;
                // Route.SetRange(Code, SPP."Current Route");
                // if Route.FindFirst then begin
                //  MESSAGE('Paybill:'+Route."Pay Bill");
                PB := 'PAYBILL'
            end;
    ...
    // local procedure FetChCurrencyCode(No: Code[20]) CCode: Code[20]
    // var
    //     GLS: Record "General Ledger Setup";
    //     InvH: Record "Sales Invoice Header";
    // begin
    //     InvH.GET(No);
    //     if InvH."Currency Code" = '' then begin
    //         GLS.FindFirst();
    //         exit(GLS."Local Currency Symbol")
    //     end
    //     else begin
    //         exit(InvH."Currency Code");
    //     end;
    // end;
